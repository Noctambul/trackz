    ...
    OwnableRoles
{
    ...
    function setEditionMaxMintableRange(
        ...
        emit EditionMaxMintableRangeSet(
    ...
    function mintRandomness() public view returns (uint256) {
        if (mintConcluded() && mintRandomnessEnabled()) {
            return
                uint256(keccak256(abi.encode(_mintRandomness, address(this))));
        }
        return 0;
    }
    ...
    modifier updatesMintRandomness() {
        if (mintRandomnessEnabled() && !mintConcluded()) {
            ...
            assembly {
                ...
                mstore(
                    0x00,
                    blockhash(sub(number(), add(1, byte(0, randomness))))
                )
        ...
        returns (string memory name_, string memory symbol_)
    {
        ...
        unchecked {
            ...
            if (packed != bytes32(0)) {
                ...
                bytes memory symbolBytes = new bytes(
                    uint8(packed[1 + nameBytes.length])
                );
                // Copy the bytes.
